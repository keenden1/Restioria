...
public class quiz_hard5 extends AppCompatActivity {
    private TextView countdownTextView;
    private CountDownTimer countDownTimer;
    private boolean isTimerPaused = false;
    private long timeRemaining;
    private TextView button_hard_1d,button_hard_2d,button_hard_3d,questionhard5;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz_hard5);
        setFullscreen();
        ...
        button_hard_1d= findViewById(R.id.button_hard_1d);
        button_hard_2d= findViewById(R.id.button_hard_2d);
        button_hard_3d= findViewById(R.id.button_hard_3d);
        questionhard5 = findViewById(R.id.question_hard_5);

        int totalQuestions = 5;
        int randomQuestionNumber = (int) (Math.random() * totalQuestions) + 1;
        String randomQuestionPath = "hard/questions5/question" + randomQuestionNumber;
        ...
        button_hard_1d.setOnClickListener(new View.OnClickListener() {
            @Override
                ...
                databaseRef.child(randomQuestionPath).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        if (questionA != null && expectedAnswer != null && questionA.equals(expectedAnswer)) {
                            button_hard_1d.setBackgroundResource(R.drawable.border3);
                            button_hard_2d.setEnabled(false);
                            button_hard_3d.setEnabled(false);
                            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                            if (currentUser != null) {
                                String userId = currentUser.getUid();
                                DatabaseReference userRef = databaseRef.child("users").child(userId);
                                userRef.child("score").child("HardLevel_5").setValue(1);
                            }
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    Intent intent = new Intent(quiz_hard5.this, score_hard.class);
                                    startActivity(intent);
                                }
                            }, 1000);
                        }else{
                            button_hard_1d.setBackgroundResource(R.drawable.border4);
                            button_hard_2d.setEnabled(false);
                            button_hard_3d.setEnabled(false);
                            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                            if (currentUser != null) {
                                String userId = currentUser.getUid();
                                DatabaseReference userRef = databaseRef.child("users").child(userId);
                                userRef.child("score").child("HardLevel_5").setValue(0);
                            }
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
        ...
        button_hard_2d.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Check the condition in Firebase Realtime Database
                DatabaseReference databaseRef = FirebaseDatabase.getInstance().getReference();
                ...
                databaseRef.child(randomQuestionPath).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        if (questionA != null && expectedAnswer != null && questionA.equals(expectedAnswer)) {
                            button_hard_2d.setBackgroundResource(R.drawable.border3);
                            button_hard_1d.setEnabled(false);
                            button_hard_3d.setEnabled(false);
                            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                            if (currentUser != null) {
                                String userId = currentUser.getUid();
                            ...
                            button_hard_2d.setBackgroundResource(R.drawable.border4);
                            button_hard_1d.setEnabled(false);
                            button_hard_3d.setEnabled(false);
                            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                            if (currentUser != null) {
        ...
        button_hard_3d.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Check the condition in Firebase Realtime Database
                DatabaseReference databaseRef = FirebaseDatabase.getInstance().getReference();
                ...
                databaseRef.child(randomQuestionPath).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        if (questionA != null && expectedAnswer != null && questionA.equals(expectedAnswer)) {
                            button_hard_3d.setBackgroundResource(R.drawable.border3);
                            button_hard_1d.setEnabled(false);
                            button_hard_2d.setEnabled(false);
                            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                            if (currentUser != null) {
                            ...
                            button_hard_3d.setBackgroundResource(R.drawable.border4);
                            button_hard_1d.setEnabled(false);
                            button_hard_2d.setEnabled(false);
                            FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                            if (currentUser != null) {
